...
<style>
    * { box-sizing: border-box; }
    ...
    /* When the name+artist are wider than the bar, scroll them as a
       marquee instead of wrapping to a second line (see updateMarquee()
       in players/main.js, which only turns this on when it's needed). */
    ...
    #player-root .marquee-track.scrolling {
        animation: player-marquee var(--marquee-duration, 8s) ease-in-out infinite;
    }

    @keyframes player-marquee {
        0%, 12% { transform: translateX(0); }
        50%, 62% { transform: translateX(var(--marquee-distance, 0)); }
